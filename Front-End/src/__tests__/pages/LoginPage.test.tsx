...
describe('LoginForm', () => {
    ...
    it('navigates to dashboard on successful login', async () => {
        ...
        fireEvent.change(screen.getByLabelText(/Email Address/i), { target: { value: 'doctor@stemsight.com' } });
        ...
        await waitFor(() => expect(mockLogin).toHaveBeenCalledWith({ email: 'doctor@stemsight.com', password: 'correct-password' }));
    ...
    it('shows unverified account message when server returns that error', async () => {
        mockLogin.mockResolvedValue({ success: false, error: 'Account not verified' });
        ...
        expect(await screen.findByText(/Account not verified/i)).toBeInTheDocument();
        expect(mockPush).not.toHaveBeenCalled();
    });
});
