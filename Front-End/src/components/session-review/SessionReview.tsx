...
const SessionReview: React.FC<SessionReviewProps> = ({ initialPatient }) => {
  ...
  const [sessionsWithPatients, setSessionsWithPatients] = useState<SessionWithPatient[]>([]);
    ...
    fetch('http://127.0.0.1:8000/patients/')
      ...
      .then((data: Patient[]) => {
        ...
        setSessions((prev) => {
          ...
          if (initialPatient) {
            console.log("Overriding first session with initial patient:", initialPatient.first_name, initialPatient.last_name);
  ...
  useEffect(() => {
    if (initialPatient) {
      console.log("Initial Patient:", initialPatient.first_name, initialPatient.last_name);
      ...
      const randomSession = sessionsWithPatients[Math.floor(Math.random() * sessionsWithPatients.length)];
      if (randomSession) {
        console.log("Randomly selected patient:", randomSession.patient?.first_name, randomSession.patient?.last_name);
        setSessionId(randomSession.session.id);
      }
    }
  }, [initialPatient, sessionsWithPatients]);

  const currentSessionObj = sessionsWithPatients.find((sp) => sp.session.id === sessionId);
