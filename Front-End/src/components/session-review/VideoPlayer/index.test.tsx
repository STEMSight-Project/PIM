...
describe("VideoPlayer Component", () => {
  ...
  describe("Rendering", () => {
    ...
    test("renders video with default test URL when videoUrl is null", () => {
      render(<VideoPlayer videoUrl={null} currentTimestamp={0} />);
    ...
    test('displays "No video available" message when src is falsy', () => {
      ...
      render(<VideoPlayer videoUrl="" currentTimestamp={0} />);
    ...
    test("displays current timestamp", () => {
      render(<VideoPlayer videoUrl="test.mp4" currentTimestamp={42.5} />);

      expect(
        screen.getByText(/Current timestamp: 42\.50 seconds/)
      ).toBeInTheDocument();
    });

    test("video has correct attributes", () => {
      render(<VideoPlayer videoUrl="test.mp4" currentTimestamp={0} />);
  ...
  describe("Timestamp Seeking", () => {
    test("seeks to timestamp when video metadata is loaded", async () => {
      const { container } = render(
        <VideoPlayer videoUrl="test.mp4" currentTimestamp={30} />
      );
    ...
    test("seeks immediately if metadata already loaded", async () => {
      const { container, rerender } = render(
        <VideoPlayer videoUrl="test.mp4" currentTimestamp={15} />
      );
      ...
      rerender(<VideoPlayer videoUrl="test.mp4" currentTimestamp={20} />);
    ...
    test("handles invalid timestamp by setting to 0", () => {
      const { container } = render(
        <VideoPlayer videoUrl="test.mp4" currentTimestamp={NaN} />
      );
    ...
    test("updates seek position when currentTimestamp prop changes", async () => {
      const { rerender, container } = render(
        <VideoPlayer videoUrl="test.mp4" currentTimestamp={10} />
      );

      const video = container.querySelector("video") as HTMLVideoElement;
      Object.defineProperty(video, "readyState", { value: 2, writable: true });

      rerender(<VideoPlayer videoUrl="test.mp4" currentTimestamp={50} />);
  ...
  describe("Time Update Callback", () => {
    ...
    test("does not crash if onTimeUpdate is not provided", () => {
      const { container } = render(
        <VideoPlayer videoUrl="test.mp4" currentTimestamp={0} />
      );
  ...
  describe("Error Handling", () => {
    test("logs error when video fails to load", () => {
      ...
      const { container } = render(
        <VideoPlayer videoUrl="invalid.mp4" currentTimestamp={0} />
      );
    ...
    test("handles seek error gracefully", () => {
      ...
      expect(() => {
        render(<VideoPlayer videoUrl="test.mp4" currentTimestamp={Infinity} />);
      }).not.toThrow();
  ...
  describe("Video Source Changes", () => {
    test("updates video src when videoUrl prop changes", () => {
      const { rerender, container } = render(
        <VideoPlayer videoUrl="video1.mp4" currentTimestamp={0} />
      );

      let video = container.querySelector("video") as HTMLVideoElement;
      expect(video).toHaveAttribute("src", "video1.mp4");

      rerender(<VideoPlayer videoUrl="video2.mp4" currentTimestamp={0} />);
    ...
    test("resets to beginning when changing videos", async () => {
      const { rerender, container } = render(
        <VideoPlayer videoUrl="video1.mp4" currentTimestamp={50} />
      );
