...
def init_seed(_):
    ...
    torch.backends.cudnn.benchmark = True  # Changed to True for auto-optimization
...
class Processor:
    ...
    def load_model(self):
        output_device = (
            self.arg.device[0]
            if type(self.arg.device) is list and len(self.arg.device) > 0
            else self.arg.device
        )
        self.output_device = output_device
        self.use_cuda = type(self.arg.device) is list and len(self.arg.device) > 0
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            if self.use_cuda:
                data = Variable(
                    data.float().cuda(self.output_device, non_blocking=True),
                    requires_grad=False,
                )
                label = Variable(
                    label.long().cuda(self.output_device, non_blocking=True),
                    requires_grad=False,
                )
            else:
            ...
            if self.use_cuda and batch_idx % 5 == 0:
                torch.cuda.empty_cache()
            ...
            for batch_idx, (data, label, index) in enumerate(process):
                with torch.no_grad():
                    if self.use_cuda:
                        ...
                        data = Variable(
                            data.float(), requires_grad=False, volatile=True
                        )
                        label = Variable(
                            label.long(), requires_grad=False, volatile=True
                        )
                    output = self.model(data)
                    if isinstance(output, tuple):
                        output, l1 = output
