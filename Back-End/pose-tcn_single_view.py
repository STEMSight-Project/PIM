...
# -*- coding: utf-8 -*-
"""
Single-View PoseTCN Trainer — POSE-ONLY + SINGLE VIEW SAMPLING

Key difference from train.py:
- ✅ Loads multi-view NPZ files but randomly selects ONE view per sample
- ✅ Uses simpler single-path TCN architecture (no multi-view fusion)
- ✅ Matches live demo inference exactly (single webcam -> single model path)
- ✅ All other training features preserved (focal loss, EMA, AMP, temporal artifacts, etc.)

Usage:
    python train_single_view.py --data_dir npz_output --out runs/single_view --T 60 --batch 384
"""
...
class NPZWindowDataset(Dataset):
    """
    SINGLE-VIEW SAMPLING: Loads multi-view NPZ files but randomly selects ONE view per sample.
    This ensures training matches single-webcam inference.
    """
    def __init__(self, files: List[str], class_map: Dict[str, int],
        ...
        self.num_views = 1  # ALWAYS 1 for single-view training
        self.input_dim = self.landmarks_per_view * 3  # Single view only
    ...
    def __getitem__(self, idx):
        ...
        # 🎯 SINGLE VIEW SELECTION: randomly pick one view
        if len(views) == 0:
            seq = np.zeros((self.T, self.landmarks_per_view, 3), np.float32)
        else:
            selected_view = views[random.randint(0, len(views) - 1)]
...
class CollateWindows:
    ...
    def __call__(self, batch):
        ...
        def _rotate_y(sample, deg):
            # (T,F) where F = L*3 -> (T,L,3) and rotate xyz
...
def main():
    ...
    try:
        ...
        print(f"✓ Files: {len(full_ds.meta_per_file)}  |  Windows: {len(full_ds)}  |  Views: {full_ds.num_views} (SINGLE-VIEW MODE)")
    ...
    print(f"✓ Single-view input_dim: {full_ds.input_dim} (33 landmarks × 3)")
    ...
    print("   ✓ Using balanced sampler (label × subject)" if use_sampler else "   ✓ Using random sampling")
    ...
    model = PoseTCNSingleView(
        ...
        in_features=full_ds.landmarks_per_view * 3,
    ...
    if args.imbalance_strategy != "weights":
        try:
            ...
            print("🧭 Initialized classifier bias with train priors.")
        except Exception as e:
            print(f"⚠️ Could not initialize head bias: {e}")
    else:
    ...
    print(f"🎯 SINGLE-VIEW MODE: Random view selection per sample (matches live demo)")
