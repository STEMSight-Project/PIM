        ...
        if results.pose_landmarks:
            ...
            if len(self.sequence_buffer) == 30:
                try:
                    ...
                    sequence = torch.tensor(np.array(list(self.sequence_buffer)), dtype=torch.float32)
                    ...
                    with torch.no_grad():
                        ...
                        if len(self.prediction_history) >= 5:
                            ...
                            if avg_confidence > 0.7:
                                most_common = max(set([pred for pred, _ in recent_predictions]), 
            ...
            self.mp_drawing.draw_landmarks(
                bgr_frame, results.pose_landmarks, self.mp_pose.POSE_CONNECTIONS,
                self.mp_drawing.DrawingSpec(color=(0, 255, 0), thickness=2, circle_radius=2),
                self.mp_drawing.DrawingSpec(color=(0, 0, 255), thickness=2)
            )
            
            # Add PIM detection text
            cv2.putText(bgr_frame, display_text, (10, 30), 
...
def default_device() -> str:
    ...
    if os_name == "Windows":
        return "Logitech HD Webcam C525"
...
def main() -> None:
    ...
    parser.add_argument("--signaling", default="http://localhost:8000", help="Signaling server URL")
    parser.add_argument("--video_device", required=False, help="Video device name or index")
    parser.add_argument("--device_name", default="PIM-Broadcaster-Clean", help="Device name")
    ...
    try:
        asyncio.run(publish(args.room, args.signaling, args.video_device, args.device_name))
    except KeyboardInterrupt:
