...
def get_events_for_patient(patient_id: str):
    try:
        ...
        logger.error("Error getting events for patient: %s", e)
...
def get_events_for_video(video_id: str):
    try:
        ...
        logger.error("Error getting events for video: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
...
def create_event(event: PatientEventBase):
    try:
        response = supabase.table("patient_event").insert(event.model_dump()).execute()
...
def update_event_status(event_id: str, status_update: StatusUpdate):
    try:
        ...
        logger.error("Error updating event status: %s", e)
        raise HTTPException(status_code=500, detail=str(e)) from e
