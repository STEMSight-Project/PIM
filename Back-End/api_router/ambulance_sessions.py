...
async def get_all_sessions():
    ...
    """
    try:
        result = supabase.table('ambulance_session_recordings').select('*').order('session_start', desc=True).execute()
...
@router.get("/{session_id}", response_model=AmbulanceSession)
async def get_session_by_id(session_id: str):
    """
    Get a specific ambulance session by ID
    """
    try:
        result = supabase.table('ambulance_session_recordings').select('*').eq('id', session_id).single().execute()
...
async def get_active_sessions():
    ...
    """
    try:
        result = supabase.table('ambulance_session_recordings').select('*').is_('session_end', 'null').order('session_start', desc=True).execute()
...
async def get_sessions_by_session_id(session_id: str):
    ...
    """
    try:
        result = supabase.table('ambulance_session_recordings').select('*').eq('session_id', session_id).execute()
        ...
        logger.exception("Failed to retrieve sessions for session_id %s: %s", session_id, exc)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch sessions"
        ) from exc
...
async def get_sessions_by_status(status_filter: str):
    ...
    """
    try:
        result = supabase.table('ambulance_session_recordings').select('*').eq('status', status_filter).order('session_start', desc=True).execute()
        ...
        logger.exception("Failed to retrieve sessions by status %s: %s", status_filter, exc)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Failed to fetch sessions"
        ) from exc