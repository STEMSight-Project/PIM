...
Version: 1.1 - Fixed session_type to use 'monitoring'
"""
...
async def create_streaming_room(room_id: str, device_name: Optional[str] = None):
    ...
    The room_id can be:
    1. A valid ambulance UUID
    2. A string containing ambulance number like "AMB-001-ROOM-001"
    3. Any other string (will look up or create ambulance based on pattern)
    """
    try:
        # Try to extract ambulance number from room_id pattern (e.g., "AMB-001-ROOM-001")
        ambulance_id = None
        ...
        try:
            ...
            logger.info("Using room_id as ambulance UUID: %s", ambulance_id)
        except ValueError:
            # Not a valid UUID, try to extract ambulance number
            import re
            # Pattern: AMB-XXX or just XXX where XXX is 001, 002, etc.
            amb_pattern = r'AMB-(\d{3})|^(\d{3})'
            match = re.search(amb_pattern, room_id)
            
            if match:
                # Get the ambulance number (either from AMB-XXX or just XXX)
                amb_number = match.group(1) or match.group(2)
                ambulance_number = f"AMB-{amb_number}"
                logger.info("Extracted ambulance number %s from room_id %s", ambulance_number, room_id)
                
                # Look up the ambulance in the database by ambulance_number
                result = supabase.table("ambulances").select("id").eq("ambulance_number", ambulance_number).execute()
                
                if result.data and len(result.data) > 0:
                    ambulance_id = result.data[0]["id"]
                    logger.info("Found ambulance ID %s for %s", ambulance_id, ambulance_number)
                else:
                    # Ambulance not found in database
                    raise HTTPException(
                        status_code=404, 
                        detail=f"Ambulance {ambulance_number} not found in database. Please create the ambulance first."
                    )
            else:
                # No pattern match, generate UUID from room_id string (fallback)
                ambulance_id = str(uuid.uuid5(uuid.NAMESPACE_DNS, room_id))
                logger.warning("No ambulance pattern found in room_id %s, generated UUID %s", room_id, ambulance_id)
        ...
        if existing_sessions:
            ...
            # Use 'monitoring' as the session type (valid options: emergency, training, maintenance, monitoring)
            session = await StreamingDatabaseService.create_ambulance_session(
                ambulance_id=ambulance_id, session_type="monitoring"
            )
